...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    InlineKeyboardButton('⚡️Aᴅᴅ Mᴇ Tᴏ Yᴏᴜʀ Gʀᴏᴜᴘ⚡️', url=f'http://t.me/{clonedme.U_NAME}?startgroup=true')
                ],[
                    InlineKeyboardButton('⚙️Hᴇʟᴘ⚙️', callback_data="help"),
                    InlineKeyboardButton('🛡️Aʙᴏᴜᴛ🛡️', callback_data="about")
                  ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await message.reply(script.CLONESTART_TXT.format(message.from_user.mention if message.from_user else message.chat.title, temp.U_NAME, temp.B_NAME), reply_markup=reply_markup)
