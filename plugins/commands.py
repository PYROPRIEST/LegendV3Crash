...
from info import CHANNELS, ADMINS, AUTH_CHANNEL, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT, CHNL_LNK, GRP_LNK, REQST_CHANNEL, SUPPORT_CHAT_ID, MAX_B_TN, CHANNEL_ID, CHANNEL_LINK, FILE_REDIRECT
...
async def start(client, message):
    ...
    if FILE_REDIRECT:
        mh = await client.send_cached_media(
            chat_id=CHANNEL_ID,
            file_id=file_id,
            caption=script.FILE_CHANNEL_TXT.format(message.from_user.mention, title, size),
            protect_content=True if pre == 'filep' else False,
            reply_markup=InlineKeyboardMarkup(
                [[                          
                  InlineKeyboardButton("ɴᴇᴡ ᴍᴏᴠɪᴇꜱ", url='https://t.me/+2p4g9JuMTBY1N2M1')
        ...
        master = await message.reply(
            script.FILE_READY_TXT.format(message.from_user.mention, title, size),
            True,
            enums.ParseMode.HTML,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
